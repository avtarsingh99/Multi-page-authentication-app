...
const Books = () => {
  ...
  return (
    <div className='flex flex-col gap-6 items-center justify-center mb-12'>
      ...
      <section className='flex flex-col gap-6 items-center justify-between bg-emerald-950 w-full py-8'>
        ...
        <div className='flex flex-col gap-14 items-center'>
          ...
          <div className='flex flex-col gap-5 items-center'>
            <h1 className='text-4xl font-bold text-amber-400'>Explore Our Books</h1>
            <p className='text-lg text-white/80 '>From emotional stories to exciting adventures, LibNest has something for every kind of reader.</p>
          </div>
      ...
      <div className='grid grid-cols-4 gap-5 max-w-full w-full px-16'>
        <BookCard />
      </div>

      <section className='flex flex-col gap-6 items-center'>
        <h2 className='text-4xl font-bold text-amber-400'>Reading Tips</h2>

        <div className='grid grid-cols-3 gap-12 items-center max-w-full w-full px-16 justify-center'>

          {readingHabits.map((habit) => (
            <div key={habit.id} className='flex flex-col gap-4 items-center justify-center px-4 py-4 border border-emerald-800 rounded-xl bg-white shadow-lg shadow-emerald-800/10 w-full'>
              <p className='font-bold text-md text-emerald-800'>{habit.title}</p>
